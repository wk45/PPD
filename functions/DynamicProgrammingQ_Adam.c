...
double CostFn2(const double *q1L, const double *q2L, const double *lam, int k, int l, int i, int j, int n, int scl);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	const int scl = 5;
	const double *q1, *q2, *lam;
	double *q1L, *q2L, *yy, *D1, *D2, *tmp1, *tmp2, *E, Etmp, Emin, t, a, b;
	if (nrhs != 4)
	...
	for (j = 1; j < N; ++j) {
		for (i = 1; i < N; ++i) {
			...
			for (Num = 0; Num < NNBRS; ++Num) {
				...
				if (k >= 0 && l >= 0) {
					Etmp = E[N*l + k] + CostFn2(q1L,q2L,lam,k,l,i,j,n,scl);
					if (Num == 0 || Etmp < Emin) {
						Emin = Etmp;
						Eidx = Num;
					}
