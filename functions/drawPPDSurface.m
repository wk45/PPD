...
IndicatorMatrix_full = double(~isnan(Heights));
...
figure
    S= surf(t,lam,FNm,'LineStyle','none','FaceAlpha',0.9,'AlphaData',1,'LineWidth',0.5);
    colormap(parula);
...
for j = 1:labelMax
    ...
    plot3(t(1) + t(LocationMatrix_full(idx_full, j)) * (t(end) - t(1)), ...
          lam(idx_full), ...
    ...
    text(t(1) + t(LocationMatrix_full(idx_full(end), j)) * (t(end) - t(1)) + x_offset, ...
    ...
    plot3(t(1) + t(LocationMatrix_sig(idx_sig, j)) * (t(end) - t(1)), ...
          lam(idx_sig), ...
